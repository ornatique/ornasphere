...
<div class="content-wrapper">
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h4 class="card-title">Tree Cutting Issue Voucher List</h4>
        </div>
        <div class="card-body">
            <div class="tree-cutting-filters">
            ...
            <div class="table-responsive tree-cutting-list-scroll">
                <table class="table table-bordered table-striped" id="treeCuttingIssueTable">
                    <thead>
                        <tr>
                            <th>Sr No</th>
...
@push('styles')
<style>
    .tree-cutting-list-scroll { max-height: calc(100vh - 360px); overflow-y: auto; }
    .tree-cutting-filters {
        display: grid;
    ...
    .tree-cutting-filters label { display: block; margin-bottom: 5px; color: #b8b8d4; font-size: 12px; }
    .filter-actions { display: flex; gap: 8px; flex-wrap: wrap; }
    .filter-actions .btn { min-width: 86px; }
    #treeCuttingIssueTable thead th { position: sticky; top: 0; z-index: 2; background: #25263a; }
    .count-badge {
        display: inline-flex;
        align-items: center;
    ...
    @media (max-width: 767px) {
        .tree-cutting-filters { grid-template-columns: 1fr; }
...
<script>
    ...
    const treeCuttingIssueTable = $('#treeCuttingIssueTable').DataTable({
        processing: true,
        serverSide: true,
        ajax: {
            url: "{{ route('company.tree-cutting-issue.index', $company->slug) }}",
            data: function (data) {
    ...
    $('#applyFilter').on('click', function () { treeCuttingIssueTable.ajax.reload(); });
    $('#resetFilter').on('click', function () {
        $('#fromDate').val(defaultFromDate);
        $('#toDate').val(defaultToDate);
        $('#workerFilter').val('');
        treeCuttingIssueTable.ajax.reload();
    });
