...
@php
    ...
    $billableItems = $approval->items
        ...
        ->sortBy(function ($row) {
            ...
            $label = strtoupper(trim((string) ($row->qr_code ?? optional($itemSet)->qr_code ?? '')));
            if ($label === '') {
                $label = strtoupper(trim((string) ($row->huid ?? optional($itemSet)->HUID ?? '')));
            }
...
<div class="sheet">
    ...
    <table class="meta">
        <tr>
            ...
            <td style="width:44%;">
                ...
                <div style="margin-top:4px;"><strong>Generated By</strong> : {{ $printedBy ?? (optional($approval->creator)->name ?? '-') }}</div>
                <div style="margin-top:4px;"><strong>Generated On</strong> : {{ isset($printedAt) ? \Carbon\Carbon::parse($printedAt)->format('d-m-Y h:i A') : now()->format('d-m-Y h:i A') }}</div>
            </td>
        </tr>
    </table>
    ...
    <table class="voucher-grid">
        <thead>
            <tr>
                <th class="sr-col" style="width:6%;">Sr</th>
