...
const barChartStackedOptions = {
    ...
    plugins: {
        legendCallback: (chart) => {
            ...
            for (let i = 0; i < chart.data.datasets.length; i++) {
                text.push('<div class="col-sm-5 me-3 ms-3 ml-sm-0 mr-sm-0 pr-md-0 mt-3"><div class="row align-items-center"><div class="col-2"><span class="legend-label" style="background-color:' + chart.data.datasets[i].backgroundColor + '"></span></div><div class="col-9"><p class="text-dark m-0">' + chart.data.datasets[i].label + '</p></div></div>');
                text.push('</div>');
            }
            text.push('</div>');
            return text.join("");
 ...
 const barChartStackedDarkOptions = {
    scales: {
        x: {
            display: false,
            stacked: true,
...
const eCommerceAnalyticOptions = {
    scales: {
        y: {
            display: true,
            grid: {
...
const eCommerceAnalyticDarkOptions = {
    scales: {
        y: {
            display: true,
            grid: {
